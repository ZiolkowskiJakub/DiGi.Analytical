...
namespace DiGi.Analytical.Building
{
    public static partial class Query
    {
        ...
        /// <para>A space contributing no shell - it carries no component, or fewer than the four faces a closed solid needs - fails the check rather than being skipped, since a space without a boundary is exactly the state this is meant to catch. A space whose components yield no polygonal face geometry at all propagates the <see cref="System.InvalidOperationException"/> thrown by <see cref="BuildingModel.GetShells{TSpace}(IEnumerable{TSpace}, DiGi.Geometry.Core.Enums.Side?, DiGi.Geometry.Core.Enums.Orientation?, DiGi.Geometry.Core.Enums.Orientation?, double)"/> instead.</para>
        /// <para><paramref name="tolerance"/> is an upper bound, not an exact value: a shell closing at a finer tolerance is enclosed. For the default criterion that is now automatic, because <see cref="Geometry.Spatial.Query.IsClosed{TPolygonalFace3D}(DiGi.Geometry.Spatial.Classes.Polyhedron{TPolygonalFace3D}?, bool, double)"/> is monotonic in tolerance - a shell closing at a finer value closes at the requested one too. The <paramref name="manifold"/> criterion is scale-relative and is not, so a shell that fails it at the requested tolerance is retried at finer ones through <see cref="Geometry.Spatial.Query.ClosingTolerance{TPolygonalFace3D}(DiGi.Geometry.Spatial.Classes.Polyhedron{TPolygonalFace3D}?, IEnumerable{double}?, bool)"/> before it is rejected.</para>
        /// </summary>
        /// <param name="buildingModel">The building model to check.</param>
