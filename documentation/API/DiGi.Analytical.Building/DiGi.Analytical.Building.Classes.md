...
The rule needs a closed envelope. On an open one - a model missing its roof, say - the parity of a face whose ray leaves through the gap is arbitrary, so such a face may be reported outward whether it is or not, and a face none of whose rays can be resolved keeps its stored normal.
